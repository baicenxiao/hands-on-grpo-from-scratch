...
def extract_numerical_answer(text: str) -> str:
    """
    Extract numerical answer from text responses like "Paul has 60 marbles left"
    or "The answer is 18.5"
    """
    import re
    
    # First try existing boxed extraction
    if "\\boxed" in text:
    ...
    patterns = [
        # "The answer is 42" or "Answer: 42"
        r'(?:the\s+)?answer\s*(?:is|:)\s*([+-]?\d*\.?\d+)',
        # "Paul has 60 marbles left"
        r'has\s+([+-]?\d*\.?\d+)\s+\w+\s+left',
        # "Therefore, 42" or "So, 42"
        r'(?:therefore|so),?\s*([+-]?\d*\.?\d+)',
        # "= 42" or "equals 42"
        r'(?:=|equals?)\s*([+-]?\d*\.?\d+)',
        # Just a number at the end
        r'([+-]?\d*\.?\d+)\s*\.?\s*$',
        # Any number in the text (fallback)
        r'([+-]?\d*\.?\d+)'
    ]
...
def r1_zero_reward_fn(response, ground_truth, fast=True):
    ...
    if "</think> <answer>" in response and "</answer>" in response:
        ...
        if isinstance(ground_truth, str):
            ...
            # First try direct grading
            is_correct = grade(model_answer, ground_truth, fast)
            
            # If direct grading fails, try extracting numerical answer from text
            if not is_correct:
                extracted_answer = extract_numerical_answer(model_answer)
                if extracted_answer is not None:
                    is_correct = grade(extracted_answer, ground_truth, fast)
                    
        elif isinstance(ground_truth, list):
            is_correct = False
            for gt in ground_truth:
                # Handle GSM8K format for each item in list
                processed_gt = str(gt) if isinstance(gt, (float, int)) else gt
                if isinstance(processed_gt, str) and "#### " in processed_gt:
                    processed_gt = processed_gt.split("#### ")[-1].strip()
                
                # First try direct grading
                gt_correct = grade(model_answer, processed_gt, fast)
                
                # If direct grading fails, try extracting numerical answer
                if not gt_correct:
                    extracted_answer = extract_numerical_answer(model_answer)
                    if extracted_answer is not None:
                        gt_correct = grade(extracted_answer, processed_gt, fast)
                
                is_correct |= gt_correct
        if is_correct:
            return {
                "format_reward": 1.0,
                "answer_reward": 1.0,
